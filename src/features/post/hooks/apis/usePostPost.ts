...
import { useCollectPostData } from '../useCollectPostData';
import { s3PutImageToUrl } from '../../apis/usePutPresignedUrl';

export const usePostPost = async (fileKeys: string[], files: File[], presignedUrls: string[]) => {
  await Promise.all(presignedUrls.map((url, idx) => s3PutImageToUrl(files[idx], url)));

  const data = useCollectPostData(fileKeys);
